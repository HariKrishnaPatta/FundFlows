...
const { Option } = Select;
const P2RSurplusFundsTransfer = ({firstName, lastName}) => {
 ...
 const handleEditRowSave = (index) => {
   if (editingIndex === index) {
     ...
     if (editedData && editedData.transactionType && editedData.documentDate && editedData.reference && editedData.paymentProject && editedData.amount ) {
 ...
 const handleDuplicateRow = (index) => {
  ...
  const editedRowData = tableData[index];
    ...
    if (editedRowData &&editedRowData.transactionType && editedRowData.documentDate && editedRowData.reference&&editedRowData.paymentProject&&editedRowData.amount) {
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(
          "https://api.p360.build:6060/v1/fundflow/p2r-surplus-fund-transfer/fetchAll", { headers });
...
export default P2RSurplusFundsTransfer;