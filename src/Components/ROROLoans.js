...
const { Option } = Select;
const ROROLoans = ({firstName, lastName}) => {
 ...
 const handleEditRowSave = (index) => {
   if (editingIndex === index) {
     ...
     if (editedData && editedData.transactionType && editedData.documentDate && editedData.reference && editedData.loanIssuedProject && editedData.amount && editedData.loanReceivedProject) {
...
const handleDuplicateRow = (index) => {
  ...
  const editedRowData = tableData[index];
    ...
    if (editedRowData &&editedRowData.transactionType && editedRowData.documentDate && editedRowData.reference&&editedRowData.loanIssuedProject&&editedRowData.amount&&editedRowData.loanReceivedProject) {
...
const postData = {
      ...
      loanIssuedProject: editedRowData.loanIssuedProject,
      loanReceivedProject: editedRowData.loanReceivedProject,
    ...
    const createdBy = firstName && lastName ? `${firstName} ${lastName}` : "";
      const currentDateTime = dayjs().format("DD-MM-YYYY HH:mm:ss");
    // For status other than "submitted," update the table data as before
    const postData = {
      transactionType: editedRowData.transactionType,
      documentDate: editedRowData.documentDate,
      year: editedRowData.year,
      ...
      if (editingIndex === index) {
        return (
          <Input
            ...
            maxLength={9}
            onChange={(e) =>
              handleCellValueChange("loanIssuedProject",e.target.value,index)
            }
          />
        );
      } else {
      ...
      if (editingIndex === index) {
        return (
          <Input
            ...
            maxLength={9} 
            onChange={(e) =>
              handleCellValueChange("loanReceivedProject",e.target.value,index)
            }
          />
        );
      } else {
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(
          "https://api.p360.build:6060/v1/fundflow/ro2roloans/fetchAll", { headers });
