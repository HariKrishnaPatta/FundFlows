...
const { Option } = Select;
const COLevis = ({firstName, lastName}) => {
...
const handleSnackbarClose = () => {
 ...
 const handleEditRowSave = (index) => {
   if (editingIndex === index) {
     ...
     if (editedData && editedData.transactionType && editedData.documentDate && editedData.reference && editedData.paymentProject && editedData.ledger && editedData.amount && editedData.receiptProject) {
...
const handleDuplicateRow = (index) => {
  ...
  const editedRowData = tableData[index];
    ...
    if (editedRowData &&editedRowData.transactionType && editedRowData.documentDate && editedRowData.reference&&editedRowData.paymentProject &&editedRowData.ledger &&editedRowData.amount&&editedRowData.receiptProject) {
      ...
      if (editingIndex === index) {
        return (
          <Select
            value={text}
            onChange={(value) =>
              handleCellValueChange("ledger", value, index)
            }
            style={{ width: 200 }}
          >
            <Option value={"Advance Tax 1%"}>
               Advance Tax 1%
            </Option>
            <Option value={"Admin Charges 4 %"}>
              Admin Charges 4 %
            </Option>
            <Option value={"Ad charges 1% B2B"}>
              Ad charges 1% B2B
            </Option>
          </Select>
        );
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(
          "https://api.p360.build:6060/v1/fundflow/colevis/getCoLevies", { headers });
